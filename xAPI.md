...
Statements are the evidence for any sort of experience or event which is to be tracked in xAPI. Statements can be expressed 
using natural language, which is extremely useful for the design process, in addition to the machine language (code)
expected in a data format.  Statements are meant to be aggregated and analyzed to provide larger meaning for the overall
experience than just the sum of its parts.  For example, a dashboard could pull in many Statements to create data 
visualizations or learning analytics applications could look for trends of successful students.
