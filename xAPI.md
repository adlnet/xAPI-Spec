...
*	Part One:	[Overview - The Experience API](#partone)  
	...
	*   2.0.	[xAPI Ecosystem](#ecosystem) 	
		*	2.1.	[Ecosystem Components](#xapi-components) 
		* 	2.2.	[Oversight and Conformance Testing](#conformance) 
		*	2.3.	[Profiles and Communities of Practice](#COPs)  	
...
*	Part Two:	[The Application Programming Interface](#parttwo)  
	...
	*	2.0.	[Serialization and JavaScript Object Notation](#json)
	*	3.0.	[Statements](#statements)  
...
*	Part Three:	[Data Processing, Validation, and Security](#partthree)  
	...
	*	[Appendices](#append3)  
		...
		*	[Appendix D: Revision History](#Appendix1A)   
...
The xAPI environment in which learning experiences are described and communicated can be thought of as an ecosystem and modeled as such.  The major components of that system are the direct major players in the system and the 

* [Activity Providers](#def_statement_provider) which are responsible for gathering and formatting the raw data about learning experiences;
* [Learning Record Stores](#def-learning-record-store) (LRS) which are responsible for storing and providing the data about the learning experiences.  In the xAPI specification, statements of learning experience are consideredto be evidence about what the learner has done or achieved.  It is the responsibility of the LRS to ensure that these statements are stored and provided back to consumers with exactly the same data that was provided.
* [Consumers](#def_statement_processor) which are responsible for gathering and formatting the information from the record stores and making it available to final users in a way that is meaningful and useful to them

As well, there are the indirect players which act as referees and determine how the game is to be played, namely:
* xAPI specification that sets out the rules of the game about how learning experiences must be described and communicated electronically;
* Conformance Testers - The ADL or organizations appointed by it that certify that software programs that are operating in the system do in fact conform to the rules of the specification
* Communities of Practice which set out the meaning of terms and norms of practice for different learning communities.  For example, the term "fire" would have a very different meaning for a Community of Practice for trauma medicine as it would for a Human Recourses Community of Practice
...
### 2.2 Oversight and Conformance Testing
For the xAPI system to function effectively, there must be a set of rules that all the players in the system agree to follow.  As more experience is gained, these rules will change over time.  The ADL is the body responsible for overseeing these rules embodied in the xAPI specification.  The ADL is also responsible for appointing organizations that are able to provide conformance testing to ensure that players in the xAPI ecosystem which claim to be adhering the specification are in fact adhering to it.
<a name="COPs" ></a>

### 2.3 Profiles and Communities of Practice
...
An important goal of this document is to provide a guide to those who want to build systems that conform to the specification and interoperable with other conformant systems and provide criteria against which conformance can be tested.  
...
An Application Programming Interface or API is a set of rules the conformance to which allow different computer programs to communicate with each other.  Part Two of this document sets out those rule for the Experience API.  Those rules follow into the following categories:
- Common Language for Exchanging Data (JSON) 
- The Grammar of Communication - Statements, MetaData and Special Types and Rules  
